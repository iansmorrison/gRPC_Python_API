...
class GenericClientStub():
    ...
    def __init__(self):
        ...
        self.grpcMessage = importlib.import_module('{0}_pb2'.format(self.proto()))
        self.grpcServe = importlib.import_module('{0}_pb2_grpc'.format(self.proto()))
                                                 
        # inherited class must provide method service() which provides name of service
        self.grpcServeStub = getattr(self.grpcServe,'{0}Stub'.format(self.service()))
        ...
        self.channel = grpc.insecure_channel(self.net())
